...
'''
def load_doc_results_by_rank(scores_fname, result_type, results_path, labels_path, full_q_dict, rank_start=1, rank_end=10, rank_type="og_rank"):
...
def main(plot, perturb_type):
    ...
    full_query_dict, selected_terms_dict = load_queries("data_mmarco_spanish/queries.jsonl", "data_mmarco_spanish/tfc1_add_{}_final_dd_corpus.json".format(perturb_type))
    print("done loading")

    # scores_csv_path = "data_mmarco_spanish/tfc1_add_{}_target_qids_scores.csv".format(perturb_type)
    ...
    if "block" in plot:
        ...
        all_block_results,  all_block_labels, all_block_qids, _ = load_all_results("block", os.path.join("results_spanish", perturb_type))
        ...
        _ = plot_blocks_plotly(all_block_segmented, labels, "result_figures_es/{}_all_block_seg.png".format(perturb_type))
    ...
    if "head_all" in plot:
        ...
        top_ranked_doc_head_results, _, _ = load_doc_results_by_rank(
            "data_mmarco_spanish/tfc1_add_{}_target_qids_scores2.csv".format(perturb_type), 
            "head", 
            os.path.join("results_spanish", perturb_type), 
            os.path.join("results_spanish", perturb_type),
            full_query_dict,
            rank_start=1, 
            rank_end=10
        ...
        _ = plot_heads(avg_top_ranked_doc_head_results, "result_figures_es/top_ranked_doc_head_results_{}.png".format(perturb_type))
        ...
        _ = plot_heads(avg_bottom_ranked_doc_head_results, "result_figures_es/bottom_ranked_doc_head_results_{}.png".format(perturb_type))
    ...
    if "head_pos" in plot:
       ...
       # scores_df = pd.read_csv(scores_csv_path)
        head_decomp_data = load_head_decomp_by_rank(
            os.path.join("results_head_decomp_mmarco", perturb_type),
            os.path.join("results_spanish", perturb_type), 
            perturb_type, 
            # scores_df, 
            heads, 
        ...
        _ = grouped_bar_chart(head_decomp_data[0].T, heads, labels, save_path=f"result_figures_es/head_decomp_top_heads_{perturb_type}_overall.png")
        _ = grouped_bar_chart(head_decomp_data[1].T, heads, labels, save_path=f"result_figures_es/head_decomp_top_heads_{perturb_type}_exists.png")
        _ = grouped_bar_chart(head_decomp_data[2].T, heads, labels, save_path=f"result_figures_es/head_decomp_top_heads_{perturb_type}_not_exists.png")
    ...
    if "head_attn" in plot:
        ...
        head_data = load_head_pattern_by_rank(
            os.path.join("results_attn_pattern_mmarco", perturb_type), 
            os.path.join("results_spanish", perturb_type), 
            perturb_type, 
            # scores_df, 
            heads, 
        ...
        _ = grouped_bar_chart(head_data[0], heads, labels, save_path=f"result_figures_es/head_attn_pattern_{perturb_type}_overall.png")
        _ = grouped_bar_chart(head_data[1], heads, labels, save_path=f"result_figures_es/head_attn_pattern_{perturb_type}_exists.png")
        _ = grouped_bar_chart(head_data[2], heads, labels, save_path=f"result_figures_es/head_attn_pattern_{perturb_type}_not_exists.png")
